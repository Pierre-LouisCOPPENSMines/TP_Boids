...
//
//For mac users 
//compile with: g++ -o nouveau_fichier main.cpp -std=c++17 -F/Library/Frameworks -framework SDL2 -Wl,-rpath,/Library/Frameworks -O3 
...
struct flock {
    ...
    boid * get_leader_boids(){
        ...
        float leader_proj = 0;
        for (const auto& boid : boids) {
            std::vector<float> relative_pos = {boid->position[0] - this->get_mean_position()[0], boid->position[1] - this->get_mean_position()[1]};

            float proj = scalar_product(relative_pos, this->get_mean_speed());
            if (proj > leader_proj) {
                leader = boid;
                leader_proj = proj;
            }
    ...
    std::vector<std::vector<float>> local_base(){
        std::vector<float> mean_speed = this->get_mean_speed();
        std::vector<float> center = this->get_leader_boids()->position;
        std::vector<float> u = {mean_speed[0], mean_speed[1]};
        std::vector<float> v = {-u[1], u[0]};
        return {center, u, v};
    }
};
...
struct V_rule : public rule {
    V_rule(boid * b, flock * flock, world * world, float angle) {
        if (angle < 0 || angle > 2 * M_PI) {
            throw std::runtime_error("angle must be between 0 and 2 * M_PI");
        }
        std::vector<std::vector<float>> base = flock->local_base();
        std::vector<float> center = base[0];
        std::vector<float> u = base[1];
        std::vector<float> v = base[2];
        std::vector<float> target_position;
        std::vector<float> relative_pos = {b->position[0] - center[0], b->position[1] - center[1]};
        if (scalar_product(relative_pos, v) < 0) {
            float u_proj = scalar_product(relative_pos, u);
            target_position = {center[0] + u_proj * u[0] + u_proj * tan(angle) * v[0], center[1] + u_proj * u[1] + u_proj * tan(angle) * v[1]};
        }
        else {
            float u_proj = scalar_product(relative_pos, u);
            target_position = {center[0] + u_proj * u[0] - u_proj * tan(angle) * v[0], center[1] + u_proj * u[1] - u_proj * tan(angle) * v[1]};
        }
        acceleration[0] = target_position[0] - b->position[0];
        acceleration[1] = target_position[1] - b->position[1];
    }
...
void world::update() {
    for (auto& flock : flocks) {
        for (auto& boid : flock->boids) {
            ...
            V_rule v_rule(boid, flock, this, M_PI/12);
            ...
            acceleration[0] += v_rule.acceleration[0] * f_v;
            acceleration[1] += v_rule.acceleration[1] * f_v;
...
int main(int argc, const char *argv[]) {
    flock flock1(n_boids, {255, 0, 0},1.5); // RED FLOCK
    flock flock2(n_boids, {0, 255, 0},2); // GREEN FLOCK
